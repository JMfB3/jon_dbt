{{ config(
    ...
    pre_hook=[
        "create stage if not exists {{ env_var('DBT_DB') }}.jons_stages.rh__crypto_orders 
            url = {{ env_var('DBT_RH_STAGE') }}crypto_orders/'
            credentials = (aws_key_id = '{{ env_var('DBT_AWS_KEY') }}', aws_secret_key = '{{ env_var('DBT_AWS_SECRET') }}');
        copy into {{ source('raw_robinhood', 'crypto_orders') }} 
        from (
            select 
                t.$1 as data,
            ...
            from @{{ env_var('DBT_DB') }}.jons_stages.rh__crypto_orders t)
...
with 
    base as (select * from {{ source('raw_robinhood', 'crypto_orders') }}),

    all_stg_data as (
        select distinct
            value['id']::string as robinhood_crypto_order_id,
            value['average_price']::number(38, 2) as average_price,
            value['currency_code']::string as ticker,
            ...
            log_ingest_date,
            row_number() over(partition by robinhood_crypto_order_id order by log_ingest_date desc) as r
        from base,
